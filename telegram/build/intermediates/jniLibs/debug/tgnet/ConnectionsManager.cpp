...
void ConnectionsManager::onConnectionDataReceived(Connection *connection, NativeByteBuffer *data,
    ...
    if (keyId == 0) {
        ...
        if (length < 24 + 32 || (length - 24) % 16 != 0 ||
...
void ConnectionsManager::processServerResponse(TLObject *message, int64_t messageId,
    ...
    if (typeInfo == typeid(TL_new_session_created)) {
        ...
        DEBUG_D("destroyed session 0x%llx (%s)", (uint64_t) response->session_id, typeInfo ==
                                                                                  typeid(TL_destroy_session_ok)
                                                                                  ? "ok"
                                                                                  : "not found");
            ...
            for (requestsIter iter = runningRequests.begin();
                ...
                if (request->respondsToMessageId(resultMid)) {
                    ...
                    if (request->onCompleteRequestCallback != nullptr) {
                        ...
                        error = hasResult ? dynamic_cast<RpcError *>(response->result.get())
                                          : nullptr;
                        TL_error *error2 = hasResult
                                           ? dynamic_cast<TL_error *>(response->result.get())
                                           : nullptr;
                        if (error != nullptr) {
                                ...
                                if (error->error_code == 500 || error->error_code < 0) {
                                    ...
                                    if (error->error_message.find(floodWait) != std::string::npos) {
                                        std::string num = error->error_message.substr(
                                                floodWait.size(),
                                                error->error_message.size() - floodWait.size());
                                    ...
                                    if (error->error_message.find(waitFailed) !=
                                        std::string::npos) {
                                        discardResponse = true;
                                        request->minStartTime = (int32_t) (
                        ...
                        if (!discardResponse) {
                            if (implicitError != nullptr || error2 != nullptr) {
                                ...
                                request->onComplete(nullptr,
                                                    implicitError != nullptr ? implicitError
                                                                             : error2,
        ...
        if (messageId != 0) {
            ...
            lastOutgoingMessageId =
                    messageId > (lastOutgoingMessageId ? messageId : lastOutgoingMessageId);
        }
...
int32_t ConnectionsManager::sendRequestInternal(TLObject *object, onCompleteFunc onComplete,
    ...
    Request *request = new Request(lastRequestToken++, connetionType, flags, datacenterId,
...
int32_t ConnectionsManager::sendRequest(TLObject *object, onCompleteFunc onComplete,
    ...
    sendRequest(object, onComplete, onQuickAck, flags, datacenterId, connetionType, immediate,
                requestToken);
    return requestToken;
}
...
void ConnectionsManager::sendRequest(TLObject *object, onCompleteFunc onComplete,
    ...
    scheduleTask(
            [&, requestToken, object, onComplete, onQuickAck, flags, datacenterId, connetionType, immediate] {
                Request *request = new Request(requestToken, connetionType, flags, datacenterId,
                                               onComplete, onQuickAck, nullptr);
                request->rawRequest = object;
...
void ConnectionsManager::sendRequest(TLObject *object, onCompleteFunc onComplete,
    ...
    scheduleTask(
            [&, requestToken, object, onComplete, onQuickAck, onWriteToSocket, flags, datacenterId, connetionType, immediate, ptr1, ptr2, ptr3] {
                ...
                Request *request = new Request(requestToken, connetionType, flags, datacenterId,
                                               onComplete, onQuickAck, onWriteToSocket);
                request->rawRequest = object;
                request->ptr1 = ptr1;
                request->ptr2 = ptr2;
                request->ptr3 = ptr3;
...
bool
ConnectionsManager::cancelRequestInternal(int32_t token, bool notifyServer, bool removeFromClass) {
    for (requestsIter iter = requestsQueue.begin(); iter != requestsQueue.end(); iter++) {
        Request *request = iter->get();
...
void ConnectionsManager::requestSaltsForDatacenter(Datacenter *datacenter) {
    if (std::find(requestingSaltsForDc.begin(), requestingSaltsForDc.end(),
                  datacenter->getDatacenterId()) != requestingSaltsForDc.end()) {
        return;
    }
    ...
    sendRequest(request, [&, datacenter](TLObject *response, TL_error *error, int32_t networkType) {
                ...
                }, nullptr, RequestFlagWithoutLogin | RequestFlagEnableUnauthorized,
                datacenter->getDatacenterId(), ConnectionTypeGeneric, true);
}
...
void ConnectionsManager::processRequestQueue(uint32_t connectionTypes, uint32_t dc) {
    ...
    for (requestsIter iter = runningRequests.begin(); iter != runningRequests.end();) {
        ...
        if (requestDatacenter == nullptr) {
            ...
            if (requestDatacenter->lastInitVersion != currentVersion && !request->isInitRequest &&
                request->rawRequest->isNeedLayer()) {
            ...
            if (!requestDatacenter->hasAuthKey()) {
                if (std::find(neededDatacenters.begin(), neededDatacenters.end(),
                              requestDatacenter) == neededDatacenters.end()) {
    ...
    for (std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter = genericMessagesToDatacenters.begin();
        ...
        if (datacenter != nullptr) {
            ...
            sendMessagesToConnectionWithConfirmation(array, datacenter->getGenericConnection(true),
                                                     needQuickAck);
        }
    }
    ...
    for (uint32_t a = 0; a < count; a++) {
        ...
        if (datacenter->getDatacenterId() != movingToDatacenterId && !datacenter->isHandshaking() &&
            !datacenter->hasAuthKey()) {
            datacenter->beginHandshake(true);
        }
    }
...
void ConnectionsManager::updateDcSettings(uint32_t dcNum, bool workaround) {
    ...
    sendRequest(request, [&, workaround](TLObject *response, TL_error *error, int32_t networkType) {
                ...
                }, nullptr, RequestFlagEnableUnauthorized | RequestFlagWithoutLogin |
...
void ConnectionsManager::applyDnsConfig(NativeByteBuffer *buffer) {
    scheduleTask([&, buffer] {
        ...
        if (config != nullptr && config->date <= currentDate && currentDate <= config->expires) {
            ...
            if (datacenter != nullptr) {
                std::vector<TcpAddress> addresses;
                for (std::vector<std::unique_ptr<TL_ipPort>>::iterator iter = config->ip_port_list.begin();
                     iter != config->ip_port_list.end(); iter++) {
